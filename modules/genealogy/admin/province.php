...
if ($nv_Request->isset_request('change_status', 'post, get')) {
    ...
    $query = 'SELECT status FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_province WHERE provinceid=' . $provinceid;
    $row = $db->query($query)->fetch();
    if (isset($row['status'])) {
        $status = ($row['status']) ? 0 : 1;
        $query = 'UPDATE ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_province SET status=' . intval($status) . ' WHERE provinceid=' . $provinceid;
        $db->query($query);
...
if ($nv_Request->isset_request('ajax_action', 'post')) {
    ...
    $content = 'NO_' . $provinceid;
    if ($new_vid > 0) {
        $sql = 'SELECT provinceid FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_province WHERE provinceid!=' . $provinceid . ' AND countryid = ' . $countryid . ' ORDER BY weight ASC';
        $result = $db->query($sql);
        $weight = 0;
        while ($row = $result->fetch()) {
            ...
            if ($weight == $new_vid)
                ++ $weight;
            $sql = 'UPDATE ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_province SET weight=' . $weight . ' WHERE provinceid=' . $row['provinceid'] . ' AND countryid=' . $countryid;
            $db->query($sql);
        }
        $sql = 'UPDATE ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_province SET weight=' . $new_vid . ' WHERE provinceid=' . $provinceid . ' AND countryid=' . $countryid;
        $db->query($sql);
        $content = 'OK_' . $provinceid;
    }
    $nv_Cache->delMod($module_name);
...
if ($nv_Request->isset_request('delete_provinceid', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($provinceid > 0 and $delete_checkss == md5($provinceid . NV_CACHE_PREFIX . $client_info['session_id'])) {
        $weight = 0;
        $sql = 'SELECT weight FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_province WHERE provinceid =' . $provinceid;
        $result = $db->query($sql);
        list ($weight) = $result->fetch(3);
        
        $db->query('DELETE FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_province  WHERE provinceid = ' . $provinceid . ' AND countryid=' . $countryid);
        if ($weight > 0) {
            $sql = 'SELECT provinceid, weight FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_province WHERE weight >' . $weight;
            $result = $db->query($sql);
            while (list ($provinceid, $weight) = $result->fetch(3)) {
                $weight --;
                $db->query('UPDATE ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_province SET weight=' . $weight . ' WHERE provinceid=' . intval($provinceid));
            }
        }
        $nv_Cache->delMod($module_name);
...
$sql = 'SELECT * FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_country WHERE status=1';
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    if (empty($row['alias'])) {
        ...
        $stmt = $db->prepare('SELECT COUNT(*) FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_province WHERE provinceid != :provinceid AND alias = :alias');
        $stmt->bindParam(':provinceid', $row['provinceid'], PDO::PARAM_STR);
        $stmt->bindParam(':alias', $row['alias'], PDO::PARAM_STR);
        $stmt->execute();
        
        if ($stmt->fetchColumn()) {
            $weight = $db->query('SELECT MAX(weight) FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_province WHERE countryid=' . $row['countryid'])->fetchColumn();
    ...
    $count = $db->query('SELECT COUNT(*) FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_province WHERE provinceid=' . $row['provinceid'])->fetchColumn();
    ...
    if (empty($error)) {
        try {
            if (empty($row['provinceid'])) {
                $stmt = $db->prepare('INSERT INTO ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_province (code, countryid, title, alias, type, weight) VALUES (:code, :countryid, :title, :alias, :type, :weight)');
                
                $weight = $db->query('SELECT max(weight) FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_province WHERE countryid=' . $row['countryid'])->fetchColumn();
                $weight = intval($weight) + 1;
                $stmt->bindParam(':weight', $weight, PDO::PARAM_INT);
            } else {
                $stmt = $db->prepare('UPDATE ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_province SET countryid = :countryid, code = :code, title = :title, alias = :alias, type = :type WHERE provinceid=' . $row['provinceid']);
            }
    ...
    $row = $db->query('SELECT * FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_province WHERE provinceid=' . $row['provinceid'])->fetch();
    if (empty($row)) {
...
if (! $nv_Request->isset_request('id', 'post,get')) {
    ...
    $db->sqlreset()
        ->select('COUNT(*)')
        ->from('' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_province');
...
if ($show_view) {
    ...
    while ($view = $sth->fetch()) {
        $view['count'] = $db->query('SELECT COUNT(*) FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_district WHERE provinceid=' . $db->quote($view['provinceid']))
            ->fetchColumn();
