...
if( isset( $array_op[1] ) )
{
	...
	$stmt = $db->prepare( 'SELECT fid ,title,alias FROM ' . $tablelocation . '_family WHERE alias= :alias' );
	$stmt->bindParam( ':alias', $alias, PDO::PARAM_STR );
	$stmt->execute();
	list( $fid, $page_title, $alias ) = $stmt->fetch( 3 );
	if( $fid > 0 )
	{
		$base_url_rewrite = $base_url = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $module_info['alias']['family'] . '/' . $alias;
		...
		$result->closeCursor();
		unset( $query, $row );
		$contents = viewfam_family_list( $item_array, $page_title );
