...
{	
	...
	elseif( ! nv_function_exists( 'nv_aleditor' ) and file_exists( NV_ROOTDIR . '/' . NV_EDITORSDIR . '/ckeditor/ckeditor.js' ) )
	{
		...
		function znv_aleditor( $textareaname, $width = '100%', $height = '450px', $val = '', $customtoolbar = '' )
		{
			global  $module_data;
	...
	if ($nv_Request->get_string('submit', 'post') != "")
	{
		...
		$check_alias = $db->query('SELECT alias FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_genealogy where alias= "' . $news_contents['alias'] . '"')->fetch();
		if(!empty($check_alias)){
			$error[] = $nv_Lang->getModule('error_alias');
		}
		if($news_contents['fid'] == 0){
			$error[] = $nv_Lang->getModule('error_fid');
		}
		...
		if (empty($error)){
			if (empty($news_contents['id']) and $news_contents['cityid'] > 0 and empty($error))
			{
				...
				if( $news_contents['id'] > 0 )
				{
					...
					$ct_query = array();
					$tbhtml = $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_bodyhtml_' . ceil( $news_contents['id'] / 2000 );
					...
					foreach( $fids as $fid )
					{
						$ct_query[] = ( int )$db->exec( 'INSERT INTO ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_' . $fid . ' SELECT * FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_genealogy WHERE id=' . $news_contents['id'] );
					}

				/* 	$stmt = $db->prepare( 'INSERT INTO ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_bodytext VALUES (' . $news_contents['id'] . ', :bodytext, :ruletext, :contenttext )' );
				...
				$sql =  "UPDATE " . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . "_" . $module_data . "_genealogy SET
				...
				if( $db->exec( $sql ) )
				{
					...
					$sql =  "UPDATE " . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . "_" . $module_data . "_" . $news_contents['fid']. " SET
						 fid=" . intval( $news_contents['fid'] ) . ",
						 listfid=" . $db->quote(  $news_contents['listfid']) . ",
						 author=" . $db->quote(  $news_contents['author']) . ",
						 patriarch=" . $db->quote(  $news_contents['patriarch']) . ",
					...
					$sql =  'UPDATE ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_bodyhtml_' . ceil( $news_contents['id'] / 2000 ) . ' SET
					...
					{
						$ct_query[] = $db->exec( 'DELETE FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_' . $fid . ' WHERE id = ' . $news_contents['id'] );
					}
					foreach( $array_fam_new as $fid )
					{
						$db->exec( 'DELETE FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_' . $fid . ' WHERE id = ' . $news_contents['id'] );
						$ct_query[] = $db->exec( 'INSERT INTO ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_' . $fid . ' SELECT * FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_genealogy WHERE id=' . $news_contents['id'] );
					}

					$sql =  'UPDATE ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_bodytext SET bodytext=' . $db->quote( $news_contents['bodytext'] ) . ', ruletext=' . $db->quote(  $post['ruletext']) . ', contenttext=' . $db->quote(  $post['contenttext'] ) . ' WHERE id =' . $news_contents['id'] ;
			...
			$news_contents['bodytext'] = $news_contents['bodyhtml'];
			$news_contents['rule'] = $news_contents['ruletext'];
			$news_contents['content'] = $news_contents['contenttext'];
	...
	$contents = create_genealogy( $news_contents, $list_users, $array_keyword, $content_comment, $data_config, $error );
}
