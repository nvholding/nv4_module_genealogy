			...
			if( $news_contents['id'] > 0 AND ($array_op[2] == "Manager") )
			{
				if ($nv_Request->get_string('submit', 'post') != "")
				{
					...
					if (!empty($post['title']) and $post['cityid'] > 0)
					{
						...
						$post_old = $db->query( 'SELECT * FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_genealogy where id=' . $news_contents['id'] )->fetch();
						...
						if( $db->exec( $sql ) )
						{
							...
							$sql =  "UPDATE " . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . "_" . $module_data . "_" . $post['fid']. " SET
								 fid=" . intval( $post['fid'] ) . ",
								 listfid=" . $db->quote(  $post['listfid']) . ",
								 author=" . $db->quote(  $post['author']) . ",
								 patriarch=" . $db->quote(  $post['patriarch']) . ",
							...
							$sql =  'UPDATE ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_bodytext SET bodytext=' . $db->quote( $post['bodytext'] ) . ', ruletext=' . $db->quote(  $post['ruletext']) . ', contenttext=' . $db->quote(  $post['contenttext'] ) . ' WHERE id =' . $news_contents['id'] ;
