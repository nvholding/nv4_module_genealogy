<?php
namespace NukeViet\Module\genealogy;
/**
...
class Location
{
    ...
    public function getArrayCountry($inArrayId = array())
    {
        ...
        $array_country = array();
        $result = $db_slave->query('SELECT * FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_genealogy_country WHERE status=1 ' . $where . ' ORDER BY weight ASC');
    ...
    public function getArrayProvince($inArrayId = array(), $countryid = 0)
    {
        ...
        include NV_ROOTDIR . '/modules/genealogy/language/' . NV_LANG_INTERFACE . '.php';
        ...
        $result = $db_slave->query('SELECT * FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_genealogy_province WHERE status=1 ' . $where . ' ORDER BY weight ASC');
        while ($row = $result->fetch()) {
            $row['name'] = ($module_config['genealogy']['allow_type'] and !empty($row['type'])) ? $row['type'] . ' ' . $row['title'] : $row['title'];
    ...
    public function getArrayDistrict($inArrayId = array(), $provinceid = 0)
    {
        ...
        $result = $db_slave->query('SELECT * FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_genealogy_district WHERE status=1 ' . $where . ' ORDER BY weight ASC');
        while ($row = $result->fetch()) {
            $row['name'] = ($module_config['genealogy']['allow_type'] and !empty($row['type'])) ? $row['type'] . ' ' . $row['title'] : $row['title'];
    ...
    public function getArrayWard($inArrayId = array(), $districtid = 0)
    {
        ...
        $array_ward = array();
        $result = $db_slave->query('SELECT * FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_genealogy_ward WHERE status=1 ' . $where . ' ORDER BY title ASC');
        while ($row = $result->fetch()) {
            $row['name'] = ($module_config['genealogy']['allow_type'] and !empty($row['type'])) ? $row['type'] . ' ' . $row['title'] : $row['title'];
