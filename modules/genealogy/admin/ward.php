...
if ($nv_Request->isset_request('mod', 'post, get')) {
	 ...
	 if($mod == 'ajax'){
		...
		$rows = $db->query('SELECT * FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_ward WHERE districtid=' . $districtid);
		 $html='';
...
if ($nv_Request->isset_request('change_status', 'post, get')) {
    ...
    $query = 'SELECT status FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_ward WHERE wardid=' . $wardid;
    $row = $db->query($query)->fetch();
    if (isset($row['status'])) {
        $status = ($row['status']) ? 0 : 1;
        $query = 'UPDATE ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_ward SET status=' . intval($status) . ' WHERE wardid=' . $wardid;
        $db->query($query);
        $content = 'OK_' . $wardid;
    }
...
if ($nv_Request->isset_request('delete_wardid', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if (! empty($wardid) and $delete_checkss == md5($wardid . NV_CACHE_PREFIX . $client_info['session_id'])) {
        $db->query('DELETE FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_ward  WHERE wardid = ' . $wardid);
        $nv_Cache->delMod($module_name);
...
$array_district = $location->getArrayDistrict(array(),array($row['provinceid']));
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    if (empty($row['alias'])) {
        ...
        $stmt = $db->prepare('SELECT COUNT(*) FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_ward WHERE wardid != :wardid AND alias = :alias');
        $stmt->bindParam(':wardid', $row['wardid'], PDO::PARAM_STR);
        $stmt->bindParam(':alias', $row['alias'], PDO::PARAM_STR);
        $stmt->execute();
        
        if ($stmt->fetchColumn()) {
            $weight = $db->query('SELECT MAX(wardid) FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_ward WHERE wardid=' . $row['wardid'] . ' AND districtid=' . $row['districtid'])->fetchColumn();
    ...
    $count = $db->query('SELECT COUNT(*) FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_ward WHERE wardid=' . $db->quote($row['wardid']))
        ->fetchColumn();
    ...
    if (empty($error)) {
        try {
            if (empty($row['wardid'])) {
                $stmt = $db->prepare('INSERT INTO ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_ward (code, districtid, title, alias, type, location) VALUES (:code, :districtid, :title, :alias, :type, :location)');
            } else {
                $stmt = $db->prepare('UPDATE ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_ward SET code = :code, districtid = :districtid, title = :title, alias = :alias, type = :type, location = :location WHERE wardid=' . $row['wardid']);
            }
    ...
    $row = $db->query('SELECT * FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_ward WHERE wardid=' . $row['wardid'])->fetch();
    if (empty($row)) {
        Header('Location: ' . NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op);
        die();
    }
    $row['countryid'] = $countryid;
    $row['provinceid'] = $db->query('SELECT provinceid FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_district WHERE districtid=' . $row['districtid'])->fetchColumn();
} else {
    $row['code'] = '';
    $row['title'] = '';
    $row['alias'] = '';
...
if (! $nv_Request->isset_request('id', 'post,get')) {
    ...
    $db->sqlreset()
        ->select('COUNT(*)')
        ->from('' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_ward');
...
if ($show_view) {
    ...
    while ($view = $sth->fetch()) {
        $view['number'] = $number ++;
        $view['count'] = $db->query('SELECT COUNT(*) FROM ' . $db_config['dbsystem'] . '.' . NV_PREFIXLANG . '_' . $module_data . '_ward WHERE wardid=' . $db->quote($view['wardid']))
            ->fetchColumn();
